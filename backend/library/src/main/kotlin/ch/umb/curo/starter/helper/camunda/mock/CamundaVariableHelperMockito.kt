 ...
 */
class CamundaVariableHelperMockito(override val delegateExecution: DelegateExecution, override val logging: Boolean = false) : CamundaVariableHelperMockInterface {
    ...
    override fun mock(variable: CamundaVariableDefinition<*>, value: Any?) {
        ...
        doReturn(AbstractTypedValue(value, null)).`when`(delegateExecution).getVariableTyped<TypedValue>(eq(variable.value), eq(true))
    }
    ...
    override fun mockWithJson(variable: CamundaVariableDefinition<*>, value: String) {
        ...
        doReturn(JsonValueImpl(value, "application/json")).`when`(delegateExecution).getVariableTyped<TypedValue>(eq(variable.value), eq(true))
    }
    ...
    override fun mock(variable: CamundaVariableListDefinition<*>, value: List<*>?) {
        ...
        doReturn(ObjectValueImpl(value, ObjectMapper().writeValueAsString(value), "application/json", value?.let { value::class.java.canonicalName } ?: "", true)).`when`(
            delegateExecution).getVariableTyped<TypedValue>(eq(variable.value), eq(true))
    }
