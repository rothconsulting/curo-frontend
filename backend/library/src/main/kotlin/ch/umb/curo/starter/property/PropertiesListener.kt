...
class PropertiesListener : ApplicationListener<ApplicationContextInitializedEvent> {
    ...
    override fun onApplicationEvent(event: ApplicationContextInitializedEvent) {
        ...
        val allowedIssuer =
            event.applicationContext.environment.getProperty("curo.auth.oauth2.allowed-issuers[0]") ?: ""
        if (authType == "oauth2" && allowedIssuer.isBlank()) {
            throw CuroPropertyException(
                "Curo configuration is missing a needed parameter",
        ...
        val verifyJwt =
            event.applicationContext.environment.getProperty("curo.auth.oauth2.verify-jwt", Boolean::class.java)
                ?: false
        ...
        if (authType == "oauth2" && verifyJwt && jwkUrl.isNotEmpty() && !isValidURL(jwkUrl)) {
            throw CuroPropertyException(
                "Curo configuration has wrong parameter",
                "curo.auth.oauth2.jwt-url",
                jwkUrl,
                "jwk-url needs to be a valid url"
            )
        }

        //resourceName needs to be set if user-federation & role usage is active
        val userFederationEnabled = event.applicationContext.environment.getProperty(
            "curo.auth.oauth2.user-federation.enabled",
            Boolean::class.java
        ) ?: false
        val resourceName =
            event.applicationContext.environment.getProperty("curo.auth.oauth2.user-federation.resource-name") ?: ""
